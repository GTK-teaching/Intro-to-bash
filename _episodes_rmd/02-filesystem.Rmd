...
Here, the computer's response is the directory in which I am editing this file. It is beneath my *home directory*.

> ## The home Directory
>
> The home directory is "all the stuff that should belong to you", as opposed to stuff that 
> belongs to other users (in a multi-user system), or things that belong to the sytem itself
> (such as the operating system programs).
>
...
> output may differ slightly, but should be generally similary.
>
> 
{: .callout}
...
> it separates directories, or files from directories. *This means that `/` cannot
> be used as a file or directory name.*
>
> Since spaces separate commands from flags and arguments, it's best not to include spaces in
> filenames either. More generally, most people use a combination of letters, numbers, and a limited
> number of other characters (like `-` and `_`) to name files. The file extension (e.g., `.pdf`)
...
own filesystem.  We can see what's in our directory by running `ls`,
...
(Again, your results will be slightly different depending on your operating
...
Here,
we can see that our directory contains **sub-directories**.
...
different ways to see directories outside of your current location; we'll start
with the simplest.  
